...
# Verification targets from docs/GROUND_TRUTH.md (anchor date 2026-06-10).
#
# Do not confuse these metrics:
#   - total_reservations (250)  = distinct bookings
#   - total_stay_rows (549)     = fact-table rows at grain reservation × stay_date
#                                 (COUNT(*) on reservations_hackathon after load)
#   - otb_room_nights (514)     = SUM(number_of_spaces) for Reserved, stay_date ≥ anchor
...
EXPECTED = {
    ...
    "total_stay_rows": 549,  # Σ stay_rows in raw extract; NOT otb_room_nights (514)
    "room_type_lookup": 3,
    "market_code_lookup": 10,
    "channel_code_lookup": 4,
}
# Post-load only (not checked by etl.extract):
VERIFY_OTB_ROOM_NIGHTS = 514
